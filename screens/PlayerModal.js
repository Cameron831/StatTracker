...
const PlayerModal = ({route, navigation}) => {
    ...
    const getTracking = async () => {
      try {
        ...
        if(!foundPlayer) {
          ...
          const response = await axios.post("https://parlazyapi.cameronharris.dev:3000/user/tracking", newPlayerData);
    ...
    const updateTrackingDatabase = async (updatedTrackingInfo) => {
      try {
        const response = await axios.put("https://parlazyapi.cameronharris.dev:3000/user/tracking", updatedTrackingInfo);
      } catch (error) {
